...
调用 `xialingguo-ip` 后，不需要你先研究设计说明。Skill 会先展示一份简短的信息表单，让你一次看懂需要准备什么；你可以一次性回答，也可以只回答已经确定的部分。信息足够后，它会自动补齐设计方案并生成可发布的高清封面。

第一次调用时可以只说：

> 调用 xialingguo-ip，帮我做一张文章封面。

随后它会优先确认 4 项：文章标题/主题、视觉风格、本次视觉重点、真人 IP。比例、截图、Logo、数据图和文字排版属于可选补充，默认使用 21:9，不会因为暂时没有这些信息而卡住。

如果你不知道选什么，可以直接说“帮我选风格”和“按主题推荐真人姿势”。Skill 会根据文章内容推荐，不要求你先理解设计术语。

## 最终会得到什么

默认交付的是一张可用于推特、公众号或文章头图的高清封面成品，比例为 21:9（也支持指定其他比例）。README 里的 9 张图片只是风格缩略图，不是生成结果，也不会被当作真人 IP 素材使用。

如果标题很长或必须零错字，Skill 会先提醒图片模型存在中文错字风险；经你同意后，才会改为“高清视觉底稿 + 后期排版文字”的模式。
...
我会自动安排真人 IP、标题、产品界面、流程、结果和装饰元素的主次关系，并生成 21:9 高清横版封面。人物位置不是固定模板，而是根据本次视觉重点自然安排。
...
- 想使用的风格标识；
- 本次视觉重点；
- 如果要真人出镜：提供一张清晰的真人 IP 图片，并说明是否要保持原表情和动作。
